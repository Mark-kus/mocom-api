...
def read_root() -> dict[str, str | List[str]]:
    return {
        ...
        "endpoints": ["/accessories", "/accessories/{id}", "/accessories/on-sale"],
    }


@app.get("/accessories", response_model=PaginatedResponse)
def get_all_accessories(page: int = 1, limit: int = 10):
    """Get all mobile accessories with pagination"""
    if page < 1:
    ...
    return PaginatedResponse(
        data=mobile_accessories[start_idx:end_idx], total=len(mobile_accessories)
    )
...
@app.get("/accessories/on-sale", response_model=PaginatedResponse)
def get_accessories_on_sale(page: int = 1, limit: int = 10):
    """Get all mobile accessories that are on sale with pagination"""
    if page < 1:
        raise HTTPException(status_code=400, detail="Page must be greater than 0")
    if limit < 1 or limit > 100:
        raise HTTPException(status_code=400, detail="Limit must be between 1 and 100")
    ...
    on_sale_accessories = [
        accessory for accessory in mobile_accessories if accessory.on_sale
    ]
    start_idx = (page - 1) * limit
    end_idx = start_idx + limit

    return PaginatedResponse(
        data=on_sale_accessories[start_idx:end_idx], total=len(on_sale_accessories)
    )


@app.get("/accessories/search", response_model=PaginatedResponse)
def search_accessories(
    ...
    """Search mobile accessories by name, price, and sale status with pagination"""
    if page < 1:
        raise HTTPException(status_code=400, detail="Page must be greater than 0")
    if limit < 1 or limit > 100:
        raise HTTPException(status_code=400, detail="Limit must be between 1 and 100")
